...
def edit_task(root, db, cursor, task_id, name, description, employee_id, due_date, status, tree):
    cursor.execute(
        f"""
        UPDATE task
...
def manager_edit_task(root, db, cursor, tree):
    ...
    # Tombol Add Task
    Back_button = Button(frame, text="Edit Task", height=2, width=10, font=('Inter', 14), command=lambda: edit_task(window, db, cursor, task_id, label_task_name_value.get(), text_task_description.get("1.0", "end-1c"), label_task_employee_value.get(), label_due_date_value.get(), values[6], tree))

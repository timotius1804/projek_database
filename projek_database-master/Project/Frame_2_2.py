...
# root = Tk()
def frame_2(root, log_name):
    root.configure(bg="white")
    root.attributes("-fullscreen", True)
    root.title("Employee Task Table")
    root.geometry("1200x800")  # Menentukan ukuran awal window

    # Label Selamat Datang
    name = log_name  # Ganti dengan nama pengguna yang sesuai
    status = "Employee"
    ...
    frame = Frame(root, borderwidth=1, relief="solid", width=1200, height=800)  # Border untuk visualisasi
    ...
    tree.column("Task Name", anchor="w", width=500)
    ...
    frame_two = Frame(root, borderwidth=0, relief="solid", width=1200, height=800, bg="white")  # Border untuk visualisasi
    frame_two.grid(row=1, column=1, padx=0, pady=20, sticky="nsew")

    # Mengatur tombol "View details"
    View_details_button = Button(frame_two, text="View details", width=45, height=7, font=('Inter', 14))
    View_details_button.grid(row=0, column=0, sticky="e", pady=10, padx=5)

    # Mengatur tombol "Mark as Done / Not Done"
    Mark_button = Button(frame_two, text="Mark as Done /\nNot Done", width=45, height=7, font=('Inter', 14))
    Mark_button.grid(row=1, column=0, sticky="e", pady=10, padx=5)

    # Menambahkan baris kosong sebelum tombol Logout
    frame_two.grid_rowconfigure(2, minsize=200)  # Menambah tinggi baris 2 untuk membuat baris kosong

    # Mengatur tombol "Logout" di pojok kanan bawah
    Logout_button = Button(frame_two, text="Logout", width=7, height=1, font=('Inter'))
    Logout_button.grid(row=3, column=0, sticky="se", pady=10, padx=5)  # Pindahkan ke baris 2
